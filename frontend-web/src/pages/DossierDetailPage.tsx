    ...
    <main className="mx-auto max-w-3xl p-8">
      ...
      <form className="mt-6 space-y-2 rounded-lg bg-white p-4 shadow" onSubmit={(e) => decide(e, "VALIDE")}>
        ...
        <div className="flex gap-2">
          <button type="submit" disabled={busy} className="rounded-md bg-emerald-700 px-3 py-2 text-sm text-white">
            {busy ? "..." : "Valider"}
          </button>
          <button type="button" disabled={busy} className="rounded-md bg-red-700 px-3 py-2 text-sm text-white" onClick={(e) => decide(e, "REJETE")}>
            Rejeter
          </button>
        </div>
      </form>
