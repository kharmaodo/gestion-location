...
export function ContratDetailPage() {
  ...
  async function voirCertificat() {
    if (!id) return;
    try {
      setCertificat(await contratsApi.certificat(id));
      setError(null);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Certificat indisponible");
    }
  }
  ...
  return (
    <main className="mx-auto max-w-2xl p-8">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {contrat.statut === "BROUILLON" && <button type="button" className="rounded-md bg-primary px-3 py-2 text-sm text-white" onClick={activer}>Activer</button>}
        {contrat.statut === "ACTIF" && <button type="button" className="rounded-md border px-3 py-2 text-sm" onClick={resilier}>Resilier</button>}
        {contrat.statut === "ACTIF" && <button type="button" className="rounded-md border px-3 py-2 text-sm" onClick={voirCertificat}>Attestation</button>}
      </div>
      {certificat && (
        <section className="mt-4 rounded-lg bg-white p-4 text-sm shadow">
          <h2 className="font-medium">{certificat.type}</h2>
          <p>{certificat.texte}</p>
          <p className="mt-2 text-slate-500">{certificat.locataire} · {certificat.unite} · {certificat.bien}</p>
        </section>
      )}
      ...
      {contrat.statut === "ACTIF" && (
        <form className="mt-6 space-y-3 rounded-lg bg-white p-4 shadow" onSubmit={avenant}>
          ...
          <button type="submit" className="rounded-md bg-primary px-4 py-2 text-sm text-white">Enregistrer avenant</button>
        </form>
      )}
    </main>
  );
